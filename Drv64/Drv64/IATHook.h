...
void* _IATHook_InterlockedExchangePointer(__in void* pAddress, __in void* pValue)
{
	...
	do
	{
		...
		pOld = InterlockedExchangePointer((PVOID*)pWriteableAddr, pValue);
...
LONG _IATHook_Single(__in IATHOOK_BLOCK* pHookBlock, __in IMAGE_IMPORT_DESCRIPTOR* pImportDescriptor, __in BOOLEAN bHook)
{
	LONG nFinalRet = -1;
...
LONG IATHook(__in void* pImageBase, __in_opt char* pszImportDllName, __in char* pszRoutineName, __in void* pFakeRoutine, __out HANDLE* Param_phHook)
{
	LONG nFinalRet = -1;
